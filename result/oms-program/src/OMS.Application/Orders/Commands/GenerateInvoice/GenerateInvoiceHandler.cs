...
public class GenerateInvoiceHandler(
    ...
    IOutboxPublisher outboxPublisher) : IRequestHandler<GenerateInvoiceCommand, Result>
{
    public async Task<Result> Handle(GenerateInvoiceCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            order.GenerateInvoice(request.InvoiceNumber, request.UpdatedBy);
