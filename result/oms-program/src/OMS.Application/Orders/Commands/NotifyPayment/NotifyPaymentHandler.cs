...
public class NotifyPaymentHandler(
    ...
    IOutboxPublisher outboxPublisher) : IRequestHandler<NotifyPaymentCommand, Result>
{
    public async Task<Result> Handle(NotifyPaymentCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            order.NotifyPayment(request.UpdatedBy);
