...
public class ReleaseOrderHandler(
    ...
    IOutboxPublisher outboxPublisher) : IRequestHandler<ReleaseOrderCommand, Result>
{
    public async Task<Result> Handle(ReleaseOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            order.ReleaseOrder(request.ReleasedBy);
